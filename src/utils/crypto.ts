...
export const CONTENT_DIGEST_CONSTANTS = {
  BASE64: "base64",
  COLON: ":",
...
export const generateDigestHeader = (
  ...
  cipher:
    | typeof CONTENT_DIGEST_CONSTANTS.SHA_256
    | typeof CONTENT_DIGEST_CONSTANTS.SHA_512
): string => {
  ...
  const hash = crypto
    .createHash(cipher)
    .update(payload)
    .digest(CONTENT_DIGEST_CONSTANTS.BASE64);

  const algo: string =
    cipher === CONTENT_DIGEST_CONSTANTS.SHA_512
      ? CONTENT_DIGEST_CONSTANTS.CONTENT_DIGEST_SHA512
      : CONTENT_DIGEST_CONSTANTS.CONTENT_DIGEST_SHA256;

  return algo + hash + CONTENT_DIGEST_CONSTANTS.COLON;
};
