...
interface IAddresses {
  ...
  readonly from?: readonly NodemailerAddress[];
  readonly "reply-to"?: readonly NodemailerAddress[];
  readonly sender?: readonly NodemailerAddress[];
...
const toMailupAddresses = (
  addresses: readonly NodemailerAddress[],
): readonly Address[] =>
...
const toMailupAttachmentBody = (content: string): readonly number[] => {
...
const toMailupAttachments = (
  attachments: readonly NodemailerAttachment[],
): readonly { [k in keyof Attachment]: unknown }[] =>
  attachments.map((att) => ({
    Body:
  ...
  return {
    ...
    send: (mail, callback): void => {
      ...
      const headers = Object.keys(
        mail.data.headers as Readonly<Record<string, string>>,
      ).map((header) => ({
        N: header,
