...
describe("MultiTransport", () => {
  ...
  it("should send all emails through one transport", async () => {
    const oneTransport: Transport = {
      name: "t1",
      send: vi.fn((mail, cb) => cb(null, { ok: true })),
      version: "0.1",
    };
  ...
  it("should send emails through three transports", async () => {
    ...
    expect(t1.send).toHaveBeenCalledWith();
    expect(t2.send).toHaveBeenCalledWith();
    expect(t3.send).toHaveBeenCalledWith();
  });

  it("should no add extra info", async () => {
    const oneTransport: Transport = {
      name: "t1",
      send: vi.fn((_, cb) => cb(null, true)),
      version: "0.1",
    };
...
describe("sendMail", () => {
  ...
  const sendMailMock = vi.fn((_, f) => f(aSendMailError, undefined));
