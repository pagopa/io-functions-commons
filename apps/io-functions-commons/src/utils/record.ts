 ...
 */
export const propertiesToArray = (input: unknown): readonly IEntry[] => {
  const addDelimiter = (a: string, b: string): string => (a ? `${a}/${b}` : b);

  const paths = (obj: unknown, head = ""): readonly IEntry[] =>
    isObject(obj)
      ? pipe(
          obj,
          Object.entries,
          RA.reduce(
            [] as readonly IEntry[], // cast required in order to avoid never[] automatic assigned type to empty array
            (output, [key, value]) =>
...
export const toArray: () => (input: unknown) => readonly IEntry[] =
  () => (input) =>
    propertiesToArray(input);
