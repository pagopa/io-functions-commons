...
export type DocumentSearchKey<
  ...
  // Quick win would be to narrow T to extend BaseModel, but that way we'd lose usage flexibility
  // Hence we omit "extends BaseModel", but we check keys to be part of "T & BaseModel"
  ModelIdKey extends keyof (BaseModel & T),
  PartitionKey extends keyof (BaseModel & T) = ModelIdKey,
> =
  ...
  Pick<BaseModel & T, ModelIdKey | PartitionKey> extends Record<
    ...
    string
  > &
    Record<PartitionKey, number | string>
...
export abstract class CosmosdbModel<
  T,
  TN extends Readonly<BaseModel & T>,
  TR extends Readonly<CosmosResource & T>,
  PartitionKey extends keyof (BaseModel & T) = CosmosDocumentIdKey,
> {
  /**
  ...
  public getCollection(
    options?: FeedOptions,
  ): TaskEither<CosmosErrors, readonly t.Validation<TR>[]> {
    return pipe(
